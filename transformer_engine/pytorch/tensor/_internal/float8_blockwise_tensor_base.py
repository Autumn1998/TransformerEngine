...
class Float8BlockwiseQTensorBase(QuantizedTensorBase):
    ...
    def _create_columnwise(self):
        ...
        if self._is_2D_scaled:
            ...
            if self._get_quantizer().all_gather_usage and self._data_format == Float8BlockScaleTensorFormat.GEMM_READY:
                # If all_gather_usage is True, the scale will not be transposed, but GEMM_READY format requires transposed scale. So we follow the GEMM_READY format to transpose the scale generated by the quantizer.
                columnwise_scale_inv_shape = (columnwise_scale_inv_shape[1], columnwise_scale_inv_shape[0])
            self._columnwise_scale_inv = torch.empty(
                columnwise_scale_inv_shape,
                dtype=self._rowwise_scale_inv.dtype,
                device=self._rowwise_scale_inv.device,
