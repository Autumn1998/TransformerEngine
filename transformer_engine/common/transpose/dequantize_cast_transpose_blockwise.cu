...
__global__ void dequantize_cast_transpose_1x32_aligned_kernel(
  ...
  // Each warp will process 4x4x32 matrix
  auto local_col_id = lane_id % 8;
  auto local_row_id = warp_id * 4 + lane_id / 8;
  auto offset_col = (block_offset_y / 4 + local_col_id);
  auto offset_row = (block_offset_x + 4 * local_row_id);
  for (auto w = warp_id; w < kTileDim / 16 && offset_row < seq_len;
       w += warp_num, offset_row += 16 * warp_num) {
    float buffer_fp32[4];
    FP8_TYPE *buffer_fp8 = (FP8_TYPE *)buffer_fp32;
...
#pragma unroll
    for (int i = 0; i < 4; i++) {
      smem_float_tile[4 * local_col_id + i][local_row_id] = buffer_fp32[i];
    }
  }
  __syncthreads();
  ...
  for (auto i = threadIdx.x; i < kTileDim; i += blockDim.x) {
    if (block_offset_y + i < hidden_dim){
        output_scale_inv[ (block_offset_y + i) * scale_output_dim + blockIdx.x] = smem_scale_inv[i];
    }
  }

  // 6. store the transposed fp16 activation to the global memory
  for(auto i = threadIdx.x; i < kTileDim * kTileDim / 4; i += blockDim.x){
    auto dest_row = block_offset_y + i / (kTileDim / 4);
    auto dest_col = block_offset_x/4 + i % (kTileDim / 4);
    if( dest_row < hidden_dim && dest_col <(seq_len/4)){
      output_fp32[ dest_row * (seq_len/4) + dest_col ] = smem_float_tile[i/(kTileDim/4)][i%(kTileDim/4)];
    }
  }
  __syncthreads();
}
...
__global__ void dequantize_cast_transpose_1x32_kernel(FP8_TYPE *input, float *input_scale_inv,
  ...
  // 2. load the fp8 activation from global memory, then transpose on the smem
  // Each warp will load 1x32 row
  for (auto x = warp_id; (x < kTileDim); x += warp_num) {
    for (auto y = lane_id; (y < kTileDim); y += kThreadsPerWarp) {
      if ((block_offset_x + x < seq_len) && (block_offset_y + y < hidden_dim))
        transposed[y][x] = input[(block_offset_x + x) * hidden_dim + (block_offset_y + y)];
      else
        transposed[y][x] = FP8_TYPE(0.0f);
...
void nvte_transpose_blockwise(NVTETensor tensor, const NVTEQuantizationConfig quant_config, transformer_engine::DType intermediate_dtype, cudaStream_t stream = 0) {
  ...
  auto te_tensor = *convertNVTETensorCheck(tensor);
...
void nvte_transpose_mxfp8(NVTETensor tensor, const NVTEQuantizationConfig quant_config,
                              transformer_engine::DType intermediate_dtype, cudaStream_t stream = 0) {
  NVTE_API_CALL(nvte_transpose_mxfp8);
  using namespace transformer_engine;
  auto te_tensor = *convertNVTETensorCheck(tensor);
  ...
  const int scale_dim = rowwise_scale_inv_shape[1];
  const int scale_output_dim = colwise_scale_inv_shape[1];
